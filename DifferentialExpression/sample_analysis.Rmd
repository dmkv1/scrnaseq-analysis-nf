...
cytoband_alterations <- inferCNV_clones %>% 
  dplyr::select(Patient_Dendrogram.Group, cytoband, cn) %>% 
  filter(cn != 2) %>% 
  mutate(
    clone = word(Patient_Dendrogram.Group, 2, sep = "_"),
    .before = everything(),
    Patient_Dendrogram.Group = NULL,
    cytoband = case_when(
      cn > 2 ~ paste(cytoband, "amp", sep = " "),
      cn < 2 ~ paste(cytoband, "del", sep = " ")
    ),
    cn_distance = abs(cn - 2)
  ) %>%
  # report only the highest CN for a given cytoband
  group_by(clone, cytoband) %>%
  slice_max(cn_distance, n = 1, with_ties = FALSE) %>%
  ungroup() %>% dplyr::select(-cn_distance) %>% 
  pivot_wider(names_from = "clone", values_from = "cn", values_fill = 2) %>% 
  column_to_rownames("cytoband") %>% 
  as.matrix()
# sort by cytoband
cytoband_alterations <- cytoband_alterations[gtools::mixedsort(rownames(cytoband_alterations)), ]

top_ha <- HeatmapAnnotation(
  Clone = colnames(cytoband_alterations),
...
cytoband_heatmap <- Heatmap(
  cytoband_alterations,
  name = "CN",
  col = cnv_colors,
...
# Draw the heatmap
draw(cytoband_heatmap)
```

```{r, fig.width=13, fig.height=2.5}
cytoband_alterations_t <- t(cytoband_alterations)

left_ha <- rowAnnotation(
  Clone = rownames(cytoband_alterations_t),
  col = list(Clone = clone_colors),
...
cytoband_heatmap_t <- Heatmap(
  cytoband_alterations_t,
  name = "CN",
  col = cnv_colors,
...
draw(cytoband_heatmap_t)
```

```{r message=FALSE, warning=FALSE, fig.width=12, fig.height=3}
# this plot needs fixing - overlapping segments throw it off

subclone_CNVmap <- inferCNV_clones %>%
  filter(chr %in% autosomal$chromosome) %>%
  dplyr::select(
    "sample" = Patient_Dendrogram.Group,
    "chromosome" = chr,
    start,
    end,
    "segmean" = cn
  ) %>%
  mutate(sample = word(sample, 2, sep = "_")) %>%
...
```{r warning=FALSE}
sce_patient_tumor$clone_label <- colData(sce_patient_tumor) %>%
  as.data.frame() %>%
  dplyr::select("patient" = Patient, Timepoint, inferCNV_subclone) %>%
  left_join(
    .,
    dplyr::select(clone_annotation, inferCNV_subclone, label),
    by = join_by(inferCNV_subclone)
  ) %>%
  pull(label) %>% 
  factor(., levels = c("DGs", "DGex"))

sce_DE <- sce_patient_tumor[, sce_patient_tumor$Timepoint == "DG"]

rowData(sce_DE)[["Unique_Symbol"]] <- make.unique(rowData(sce_DE)[["Symbol"]])
rownames(sce_DE) <- rowData(sce_DE)[["Unique_Symbol"]]
seurat <- as.Seurat(sce_DE, counts = "counts", data = "logcounts")

Idents(seurat) <- seurat$clone_label
...
markers <- scoreMarkers(sce_DE, groups = sce_DE$clone_label, full.stats = TRUE)
markers <- markers[["DGs"]] %>%
  as.data.frame() %>%
  rownames_to_column("gene") %>%
  dplyr::select(gene,
...
ggplot(markers, aes(x = mean.logFC.cohen, y = mean.AUC)) +
  ...
  labs(y = "mean AUC", x = "mean Cohen's d")
```
...
gene_set <- "HALLMARK_OXIDATIVE_PHOSPHORYLATION"
...
```{r, fig.width=6, fig.height=4}
gene_set <- "REACTOME_BETA_CATENIN_INDEPENDENT_WNT_SIGNALING"
...
```{r}
outdir <- file.path("results", patient)
