...
params:
  ...
  compartment: "PBMC"
  replicate: "1"
  counts_dir: "../../alignment/output/P009_DG_PBMC_rep1/P009_DG_PBMC_rep1_Solo.out/Gene/raw"
  FDR_thresh: 0.01
...
```{r load_counts}
sample_path <- file.path(counts_dir)
...
# Add metadata
metadata(sce_unfiltered)$Sample <- sample_id
metadata(sce_unfiltered)$Patient <- patient_id
metadata(sce_unfiltered)$Timepoint <- timepoint
metadata(sce_unfiltered)$Compartment <- compartment
metadata(sce_unfiltered)$Replicate <- replicate
...
sce_unfiltered[["Replicate"]] <- replicate
...
```{r write_droplets}
# skipping as this matrix is not used
