...
cell_type_df <- colData(sce) %>% 
  ...
  mutate(
    cell_type_manual = case_when(
      clusters %in% c(2, 5, 12, 13) ~ "NK/T cells",
...
sce_B <- sce[, sce$cell_type_manual == "B cells"]
...
cell.types <- c("MCL cells", "B cells")

CCND1_logcount_threshold <- 0.5

cell.types <- c("B cells", "MCL cells")
...
cell_types <- colData(sce) %>% 
  ...
  mutate(
    cell_type_manual = case_when(
      cell_type_manual == "B cells" ~ Bcell_type,
      TRUE ~ cell_type_manual
    )
  )
cell_types$cell_type_manual %>% unique()
```
...
sce_n <- sce[, sce$cell_type_manual %in% c("B cells", "Other")]
